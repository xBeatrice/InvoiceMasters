...
const register=()=>{
	...
	.catch((err) => {
		alert("Adresa de mail nu exista sau deja a fost creat un cont cu aceasta adresa.")
...
const newTemplate = (name, description, date, currentUser, projectId) => {
 	...
 	.then((queryResult)=> console.log("Document written with ID: ", queryResult.id))
 	.catch((err) => {
		console.log(err.code)
		console.log(err.message)
	})
...
const newCompany = (companyName, companyAdress, companyVAT, companyIBAN, currentUser) => {
 	return db.collection('companyInfo')
 	.add({
 		companyName, 
 		companyAdress, 
 		companyVAT, 
...
const newVendor = (vendorName, vendorAdress, vendorVAT, vendorEmail, currentUser) => {
...
const newProduct = (productName, productDescription, productPrice, productStock, currentUser) => {
...
const updateVendor = async(vendorId, name, adress, VAT, email,)=>{
	...
	db.collection("vendors").doc(vendorId).set({vendorName: name, vendorAdress: adress, vendorVAT: VAT, vendorEmail: email, currentUser: currentUser});
} 
const updateProduct = async(productId, name, description, price, stock,)=>{
	
	db.collection("products").doc(productId).set({productName: name, productDescription: description, productPrice: price, productStock: stock, currentUser: currentUser});
} 
const getCompanyInfo = () => db.collection('companyInfo').get()

const updateCompanyInfo = async(companyId, name, adress, VAT, IBAN, currentUser)=>{
	
	db.collection("companyInfo").doc(companyId).set({companyName: name, companyAdress: adress, companyVAT: VAT, companyIBAN: IBAN, currentUser: currentUser});
} 
...
const editProjectData = async(projectId, today, newName, newDescription)=>{
	...
	db.collection("projects").doc(projectId).set({name: newName, description: newDescription, date: today, currentUser: currentUser});
}

const editTemplateData = async(templateId, today, newName, newDescription, projectId)=>{
	
	db.collection("templates").doc(templateId).set({name: newName, description: newDescription, date: today, currentUser: currentUser, projectId: projectId});
}
const modifyTemplateContent = async(templateId, templateContent)=>{
	db.collection("templates").doc(templateId).set({content: templateContent}, { merge: true })
}
